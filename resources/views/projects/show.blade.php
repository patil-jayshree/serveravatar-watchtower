...
<div class="py-8">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {{-- Request Summary (only if connected) --}}
        @if($project->is_connected)
        @php
            $totalRequests = $project->requestEvents()->count();
        ...
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6 mb-6">
            <div class="flex items-center justify-between mb-4">
                <h2 class="text-lg font-semibold text-gray-900 dark:text-white">Request Monitoring</h2>
                <a href="{{ route('organizations.projects.requests.index', [$organization, $project]) }}" class="text-sm text-primary-600 hover:text-primary-700 dark:text-primary-400 dark:hover:text-primary-300">
                    View All →
                </a>
            </div>
            <div class="grid grid-cols-2 md:grid-cols-4 gap-6">
                ...
                <div>
                    ...
                    <p class="text-2xl font-bold text-green-600 dark:text-green-400 mt-1">{{ number_format($successRequests) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Errors</p>
                    <p class="text-2xl font-bold text-red-600 dark:text-red-400 mt-1">{{ number_format($errorRequests) }}</p>
                </div>
                <div>
                    <p class="text-sm text-gray-500 dark:text-gray-400">Avg Response Time</p>
                    <p class="text-2xl font-bold text-gray-900 dark:text-white mt-1">{{ $avgDuration }} ms</p>
                </div>
            </div>
        </div>
