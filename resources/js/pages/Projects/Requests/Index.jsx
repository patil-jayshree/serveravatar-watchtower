...
import { Activity, Search, X, RefreshCw, Clock, AlertCircle, CheckCircle, XCircle, ChevronLeft, ChevronRight, ExternalLink } from 'lucide-react';
...
const METHOD_COLORS = {
    ...
    OPTIONS: 'bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-400',
};
...
export default function RequestsIndex() {
    const { organization, project, requests, stats, filters, pagination } = usePage().props;
    ...
    const tabs = [
        ...
        { key: 'success', label: 'Success', count: stats?.successful || 0, color: 'text-emerald-600 dark:text-emerald-400' },
    ...
    const applyFilters = (extraParams = {}) => {
        ...
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/requests`, params, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clearFilters = () => {
        ...
        setStatus('');
        setEnvironment('');
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/requests`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleRefresh = () => {
        setRefreshing(true);
        router.reload({ only: ['requests', 'stats', 'pagination'] });
    ...
    const hasActiveFilters = search || method || status || environment;
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Monitor HTTP requests for {project?.name}
                            </p>
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Successful</span>
                            </div>
                            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">{stats?.successful || 0}</p>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            ...
                            <p className="text-2xl font-bold text-cyan-600 dark:text-cyan-400">{formatDuration(stats?.avg_duration || 0)}</p>
                        </div>
                    </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Method</th>
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Path</th>
                                        ...
                                        {requests.map((req) => (
                                            <tr key={req.id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${getMethodColor(req.method)}`}>
                                                        {req.method}
                                                    </span>
                                                </td>
                                                <td className="px-5 py-4">
                                                    <div className="flex items-center gap-2">
                                                        <Link
                                                            href={`/organizations/${organization?.uuid}/projects/${project?.uuid}/requests/${req.uuid}`}
                                                            className="text-sm font-mono text-gray-900 dark:text-white hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors truncate max-w-md"
                                                        >
                                                            {req.path}
                                                        </Link>
                                                        {req.error_type && (
                                                    ...
                                                    {req.url && req.url !== `http://${req.host}${req.path}` && (
                                                        <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 truncate max-w-md">{req.url}</p>
                                                    )}
                                                </td>
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${getStatusColor(req.status_code)}`}>
                                                        {req.status_code}
                                                    </span>
                                                </td>
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                ...
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-gray-500 dark:text-gray-400">
                                                        ...
                                                        <span>{formatTime(req.requested_at)}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters
                                        ...
                                        : 'Your HTTP requests will appear here once captured'}
                                </p>
                            </div>
                        )}
                        ...
                        {pagination && pagination.last_page > 1 && (
                            <div className="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-slate-700">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    Showing {((pagination.current_page - 1) * pagination.per_page) + 1} to {Math.min(pagination.current_page * pagination.per_page, pagination.total)} of {pagination.total} requests
                                </p>
                                <div className="flex items-center gap-1">
                                    <button
