...
import { Clock, RefreshCw, ChevronLeft, ChevronRight, CheckCircle, XCircle, PlayCircle, Search, X } from 'lucide-react';
...
export default function JobsIndex() {
    const { organization, project, jobs, stats, filters, pagination } = usePage().props;

    const [search, setSearch] = useState(filters?.search || '');
    const [status, setStatus] = useState(filters?.status || '');
    const [queue, setQueue] = useState(filters?.queue || '');
    ...
    const tabs = [
        ...
        { key: 'started', label: 'Running', count: stats?.running || 0, color: 'text-blue-600 dark:text-blue-400' },
    ];
    ...
    const applyFilters = (extraParams = {}) => {
        ...
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/jobs`, params, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clearFilters = () => {
        ...
        setStatus('');
        setQueue('');
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/jobs`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleRefresh = () => {
        setRefreshing(true);
        router.reload({ only: ['jobs', 'stats', 'pagination'] });
    ...
    const hasActiveFilters = search || status || queue;
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Monitor queued jobs for {project?.name}
                            </p>
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Jobs</span>
                            </div>
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Job</th>
                                            ...
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Queue</th>
                                            ...
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Queued</th>
                                        </tr>
                                    </thead>
                                    <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                        {jobs.map((job) => (
                                            <tr key={job.uuid} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                                <td className="px-5 py-4">
                                                    <Link
                                                        href={`/organizations/${organization?.uuid}/projects/${project?.uuid}/jobs/${job.uuid}`}
                                                        className="block"
                                                    >
                                                ...
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${getStatusColor(job.status)}`}>
                                                        {job.status}
                                                    </span>
                                                </td>
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                ...
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-gray-500 dark:text-gray-400">
                                                        ...
                                                        <span>{formatTime(job.queued_at)}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters
                                        ...
                                        : 'Your queued jobs will appear here once captured'}
                                </p>
                            </div>
                        )}
                        ...
                        {pagination && pagination.last_page > 1 && (
                            <div className="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-slate-700">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    Showing {((pagination.current_page - 1) * pagination.per_page) + 1} to {Math.min(pagination.current_page * pagination.per_page, pagination.total)} of {pagination.total} jobs
                                </p>
                                <div className="flex items-center gap-1">
                                    <button
