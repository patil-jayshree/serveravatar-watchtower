...
import { Database, Search, X, RefreshCw, Clock, ChevronLeft, ChevronRight, AlertTriangle } from 'lucide-react';
...
export default function QueriesIndex() {
    const { organization, project, queries, stats, filters, pagination } = usePage().props;

    const [search, setSearch] = useState(filters?.search || '');
    const [type, setType] = useState(filters?.type || '');
    const [connection, setConnection] = useState(filters?.connection || '');
    const [slow, setSlow] = useState(filters?.slow || '');
    ...
    const applyFilters = (extraParams = {}) => {
        ...
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/queries`, params, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const clearFilters = () => {
        ...
        setConnection('');
        setSlow('');
        router.get(`/organizations/${organization?.uuid}/projects/${project?.uuid}/queries`, {}, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    const handleRefresh = () => {
        setRefreshing(true);
        router.reload({ only: ['queries', 'stats', 'pagination'] });
    ...
    const hasActiveFilters = search || type || connection || slow;
                    ...
                    <div className="flex items-center justify-between mb-6">
                        <div>
                            ...
                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                Monitor database queries for {project?.name}
                            </p>
                        </div>
                        <button
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Total Queries</span>
                            </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            <div className="flex items-center gap-3 mb-3">
                                ...
                                <span className="text-sm font-medium text-gray-500 dark:text-gray-400">Slow Queries</span>
                            </div>
                            <p className="text-2xl font-bold text-amber-600 dark:text-amber-400">{stats?.slow || 0}</p>
                        </div>
                        ...
                        <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 p-5">
                            ...
                            <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats?.connections || 0}</p>
                        </div>
                    </div>
                    ...
                    <div className="bg-white dark:bg-slate-800 rounded-xl border border-gray-200 dark:border-slate-700 mb-6">
                        ...
                        <div className="p-5">
                            <div className="flex items-center gap-3 flex-wrap">
                                ...
                                <select
                                    value={slow}
                                    onChange={(e) => handleFilterChange('slow', e.target.value)}
                                    className="px-3 py-2.5 bg-gray-50 dark:bg-slate-900 border border-gray-200 dark:border-slate-700 rounded-lg text-sm text-gray-600 dark:text-gray-400 focus:outline-none focus:ring-2 focus:ring-cyan-500"
                                >
                                ...
                                <table className="w-full">
                                    <thead>
                                        <tr className="border-b border-gray-200 dark:border-slate-700 bg-gray-50 dark:bg-slate-900/50">
                                            ...
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">SQL</th>
                                            <th className="px-5 py-3.5 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider">Connection</th>
                                    ...
                                    <tbody className="divide-y divide-gray-100 dark:divide-slate-700">
                                        {queries.map((query) => (
                                            <tr key={query.uuid} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold ${getQueryTypeColor(query.query_type)}`}>
                                                        {query.query_type?.toUpperCase() || 'OTHER'}
                                                    </span>
                                                </td>
                                                <td className="px-5 py-4">
                                                    <Link
                                                        href={`/organizations/${organization?.uuid}/projects/${project?.uuid}/queries/${query.uuid}`}
                                                        className="text-sm font-mono text-gray-900 dark:text-white hover:text-cyan-600 dark:hover:text-cyan-400 transition-colors truncate max-w-lg block"
                                                        title={query.sql}
                                                    >
                                                        {query.normalized_sql || query.sql}
                                                    </Link>
                                                    {query.is_slow && (
                                                        <span className="inline-flex items-center gap-1 mt-1 px-2 py-0.5 rounded text-xs font-medium bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-400">
                                                ...
                                                <td className="px-5 py-4 whitespace-nowrap">
                                                    <div className="text-sm text-gray-500 dark:text-gray-400">
                                                        <span>{formatDate(query.occurred_at)}</span>
                                                        <span className="mx-1">·</span>
                                                        <span>{formatTime(query.occurred_at)}</span>
                                                    </div>
                                                </td>
                                            </tr>
                                        ))}
                            ...
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-500 dark:text-gray-400">
                                    {hasActiveFilters
                                        ...
                                        : 'Your database queries will appear here once captured'}
                                </p>
                            </div>
                        )}
                        ...
                        {pagination && pagination.last_page > 1 && (
                            <div className="flex items-center justify-between px-5 py-4 border-t border-gray-200 dark:border-slate-700">
                                <p className="text-sm text-gray-500 dark:text-gray-400">
                                    Showing {((pagination.current_page - 1) * pagination.per_page) + 1} to {Math.min(pagination.current_page * pagination.per_page, pagination.total)} of {pagination.total} queries
                                </p>
